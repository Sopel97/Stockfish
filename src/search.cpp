...
public:
    ...
    ProbedTTEntry(TranspositionTable& tt, const Position& pos, int ply, Move ttMoveOverride) {
        ...
        // memcpy to a local first to minimize races
        TTEntry tte;
        std::memcpy(&tte, entryPtr, sizeof(TTEntry));

        ttValue   = hit ? value_from_tt(tte.value(), ply, pos.rule50_count()) : VALUE_NONE;
        ttMove    = ttMoveOverride != Move::none() ? ttMoveOverride
                  : hit ? tte.move()
                  : Move::none();
        ttCapture = ttMove && pos.capture_stage(ttMove);

        ttDepth = hit ? tte.depth() : DEPTH_UNSEARCHED;
        ttBound = hit ? tte.bound() : BOUND_NONE;
        ttEval  = hit ? tte.eval() : VALUE_NONE;
        ttPv    = hit && tte.is_pv();
    }
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    ProbedTTEntry tte(tt, pos, ss->ply, rootNode  ? thisThread->rootMoves[thisThread->pvIdx].pv[0] : Move::none());
    ss->ttHit = tte.hit;

    // At this point, if excluded, skip straight to step 6, static eval. However,
    // to save indentation, we list the condition in all code between here and there.
    if (!excludedMove)
        ss->ttPv = PvNode || tte.ttPv;

    // At non-PV nodes we check for an early TT cutoff
    if (!PvNode && !excludedMove && tte.ttDepth > depth - (tte.ttValue <= beta)
        && tte.ttValue != VALUE_NONE  // Possible in case of TT access race or if !ttHit
        && (tte.ttBound & (tte.ttValue >= beta ? BOUND_LOWER : BOUND_UPPER)))
    {
        // If ttMove is quiet, update move sorting heuristics on TT hit (~2 Elo)
        if (tte.ttMove && tte.ttValue >= beta)
        {
            // Bonus for a quiet ttMove that fails high (~2 Elo)
            if (!tte.ttCapture)
                update_quiet_stats(pos, ss, *this, tte.ttMove, stat_bonus(depth));
    ...
    else if (ss->ttHit)
    {
        ...
        unadjustedStaticEval = tte.ttEval;
        ...
        if (tte.ttValue != VALUE_NONE && (tte.ttBound & (tte.ttValue > eval ? BOUND_LOWER : BOUND_UPPER)))
            eval = tte.ttValue;
    }
    else
    {
    ...
    if (!ss->ttPv && depth < 12
        && eval - futility_margin(depth, cutNode && !ss->ttHit, improving, opponentWorsening)
               - (ss - 1)->statScore / 248
             >= beta
        && eval >= beta && eval < VALUE_TB_WIN_IN_MAX_PLY && (!tte.ttMove || tte.ttCapture))
    ...
    if (cutNode && depth >= 8 && (!tte.ttMove || tte.ttBound == BOUND_UPPER))
        depth -= 1 + !tte.ttMove;
    ...
    if (
      ...
      && !(tte.ttDepth >= depth - 3 && tte.ttValue != VALUE_NONE && tte.ttValue < probCutBeta))
    {
        assert(probCutBeta < VALUE_INFINITE && probCutBeta > beta);

        MovePicker mp(pos, tte.ttMove, probCutBeta - ss->staticEval, &thisThread->captureHistory);
...
moves_loop:  // When in check, search starts here
    ...
    if (ss->inCheck && !PvNode && tte.ttCapture && (tte.ttBound & BOUND_LOWER)
        && tte.ttDepth >= depth - 4 && tte.ttValue >= probCutBeta
        && std::abs(tte.ttValue) < VALUE_TB_WIN_IN_MAX_PLY && std::abs(beta) < VALUE_TB_WIN_IN_MAX_PLY)
        return probCutBeta;
    ...
    MovePicker mp(pos, tte.ttMove, depth, &thisThread->mainHistory, &thisThread->captureHistory,
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == tte.ttMove && !excludedMove
                && depth >= 4 - (thisThread->completedDepth > 38) + ss->ttPv
                && std::abs(tte.ttValue) < VALUE_TB_WIN_IN_MAX_PLY && (tte.ttBound & BOUND_LOWER)
                && tte.ttDepth >= depth - 3)
            {
                Value singularBeta  = tte.ttValue - (58 + 64 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                {
                    int doubleMargin = 304 * PvNode - 203 * !tte.ttCapture;
                    int tripleMargin = 117 + 259 * PvNode - 296 * !tte.ttCapture + 97 * ss->ttPv;
            ...
            else if (PvNode && move == tte.ttMove && move.to_sq() == prevSq
        ...
        if (ss->ttPv)
            r -= 1 + (tte.ttValue > alpha) + (tte.ttDepth >= depth);
        ...
        if (cutNode)
            r += 2 - (tte.ttDepth >= depth && ss->ttPv)
               + (!ss->ttPv && move != tte.ttMove && move != ss->killers[0]);
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                if (value >= beta)
                {
                    ss->cutoffCnt += 1 + !tte.ttMove - (extension >= 2);
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    Depth ttDepthThreshold = ss->inCheck || depth >= DEPTH_QS_CHECKS ? DEPTH_QS_CHECKS : DEPTH_QS_NORMAL;
    ...
    // At non-PV nodes we check for an early TT cutoff
    if (!PvNode && tte.ttDepth >= ttDepthThreshold
        && tte.ttValue != VALUE_NONE  // Only in case of TT access race or if !ttHit
        && (tte.ttBound & (tte.ttValue >= beta ? BOUND_LOWER : BOUND_UPPER)))
        return tte.ttValue;
    ...
    else
    {
        if (ss->ttHit)
        {
            ...
            if (std::abs(tte.ttValue) < VALUE_TB_WIN_IN_MAX_PLY
                && (tte.ttBound & (tte.ttValue > bestValue ? BOUND_LOWER : BOUND_UPPER)))
                bestValue = tte.ttValue;
        }
        else
        {
