...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    else if (ss->ttHit)
    {
        ...
        if (eval == VALUE_NONE)
            ss->staticEval = eval = evaluate(pos, pos.was_capture_or_promotion());
    ...
    else
    {
        if (ss->ttHit)
        {
            ...
            if ((ss->staticEval = bestValue = tte->eval()) == VALUE_NONE)
                ss->staticEval = bestValue = evaluate(pos, pos.was_capture_or_promotion());
            ...
            (ss-1)->currentMove != MOVE_NULL ? evaluate(pos, pos.was_capture_or_promotion())
                                             : -(ss-1)->staticEval;
