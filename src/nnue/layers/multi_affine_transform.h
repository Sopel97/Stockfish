...
// Definition of layer SemiAffineTransform of NNUE evaluation function

#ifndef NNUE_LAYERS_MULTI_AFFINE_TRANSFORM_H_INCLUDED
#define NNUE_LAYERS_MULTI_AFFINE_TRANSFORM_H_INCLUDED
...
namespace Stockfish::Eval::NNUE::Layers {
    ...
    // A semi affine transformation layer
    // This one performs multiple smaller affine transformations
    // They are performed in blocks of
    // InputStride -> OutputDimensions / (InputDimensions / InputStride)
    template <typename PreviousLayer, IndexType InputStride, IndexType OutputDimensions>
    class MultiAffineTransform {
    public:
        // Input/output type
        ...
        static_assert(kInputDimensions % InputStride == 0);
        static_assert(OutputDimensions % (kInputDimensions / InputStride) == 0);

        static constexpr IndexType kOutputDimensions = OutputDimensions;
        static constexpr IndexType kPaddedInputDimensions =
            CeilToMultiple<IndexType>(kInputDimensions, kMaxSimdWidth);

        static constexpr IndexType kInputStride = InputStride;

        static constexpr IndexType kPaddedInputStride =
            CeilToMultiple<IndexType>(kInputStride, kMaxSimdWidth);

        static constexpr IndexType kNumBlocks = (kInputDimensions / kInputStride);

        static constexpr IndexType kOutputStride = kOutputDimensions / kNumBlocks;

        // We cannot apply padding here
        static_assert(kPaddedInputStride % kMaxSimdWidth == 0);
        ...
        static constexpr std::uint32_t GetHashValue() {
            std::uint32_t hash_value = 0xE303BA24u;
            hash_value += kOutputDimensions + kInputStride * 7 + kOutputStride * 3;
       ...
       // Read network parameters
        bool ReadParameters(std::istream& stream) {
            ...
            {
                for (std::size_t i = 0; i < kOutputStride; ++i)
                    biases_[b * kOutputStride + i] = read_little_endian<BiasType>(stream);

                for (std::size_t i = 0; i < kOutputStride * kPaddedInputStride; ++i)
                    weights_[b * kOutputStride * kPaddedInputStride + i] = read_little_endian<WeightType>(stream);
            }
    ...
    for (IndexType block = 0; block < kNumBlocks; ++block) {

#if defined (USE_AVX512)

      constexpr IndexType kNumChunks256 = kPaddedInputStride / kSimdWidth;

      const auto output = reinterpret_cast<OutputType*>(buffer) + block * kOutputStride;

      [[maybe_unused]] const auto input_vector256 = reinterpret_cast<const __m256i*>(input + block * kInputStride);

      if constexpr (kOutputStride % 16 == 0 && kNumChunks256 == 1)
      {
        for (IndexType i = 0; i < kOutputStride; i += 16)
        {
          const IndexType offset01a = (i + 0) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;
          const IndexType offset23a = (i + 2) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;
          const IndexType offset45a = (i + 4) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;
          const IndexType offset67a = (i + 6) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;
          const IndexType offset01b = (i + 8) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;
          const IndexType offset23b = (i + 10) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;
          const IndexType offset45b = (i + 12) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;
          const IndexType offset67b = (i + 14) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;

          const __m512i bias = *reinterpret_cast<const __m512i*>(&biases_[i + block * kOutputStride]);
      ...
      constexpr IndexType kNumChunks = kPaddedInputStride / kSimdWidth;

      const auto output = reinterpret_cast<OutputType*>(buffer) + block * kOutputStride;
      const auto input_vector = reinterpret_cast<const __m256i*>(input + block * kInputStride);

      // kOutputDimensions is either 1 or a multiple of kSimdWidth
      // because then it is also an input dimension.
      if constexpr (kOutputStride % 4 == 0)
      {
        for (IndexType i = 0; i < kOutputStride; i += 4)
        {
          const IndexType offset0 = (i + 0) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;
          const IndexType offset1 = (i + 1) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;
          const IndexType offset2 = (i + 2) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;
          const IndexType offset3 = (i + 3) * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;

          const __m128i bias = *reinterpret_cast<const __m128i*>(&biases_[i + block * kOutputStride]);
      ...
      auto output = reinterpret_cast<OutputType*>(buffer) + block * kOutputStride;
      const auto input_vector = reinterpret_cast<const __m128i*>(input + block * kInputStride);
...
// Use old implementation for the other architectures.
      ...
      const auto input_vector = reinterpret_cast<const __m64*>(input + block * kInputStride);

#elif defined(USE_NEON)
      constexpr IndexType kNumChunks = kPaddedInputStride / kSimdWidth;
      const auto input_vector = reinterpret_cast<const int8x8_t*>(input + block * kInputStride);
#endif

      for (IndexType i = 0; i < kOutputStride; ++i) {
        const IndexType offset = i * kPaddedInputStride + block * kPaddedInputStride * kOutputStride;

#if defined(USE_SSE2)
        __m128i sum_lo = _mm_cvtsi32_si128(biases_[i + block * kOutputStride]);
...
#elif defined(USE_MMX)
        __m64 sum_lo = _mm_cvtsi32_si64(biases_[i + block * kOutputStride]);
        __m64 sum_hi = kZeros;
...
#elif defined(USE_NEON)
        int32x4_t sum = {biases_[i + block * kOutputStride]};
...
#else
        OutputType sum = biases_[i + block * kOutputStride];
        for (IndexType j = 0; j < kInputDimensions; ++j) {
          sum += weights_[offset + j] * input[j + block * kInputStride];
        }
        output[i] = sum;
#endif
    ...
    private:
        ...
        alignas(kCacheLineSize) WeightType weights_[kOutputDimensions * kPaddedInputStride];
    };

}  // namespace Eval::NNUE::Layers

#endif // #ifndef NNUE_LAYERS_MULTI_AFFINE_TRANSFORM_H_INCLUDED
