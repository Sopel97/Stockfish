#ifndef _NNUE_TRAINER_SEMI_AFFINE_TRANSFORM_H_
#define _NNUE_TRAINER_SEMI_AFFINE_TRANSFORM_H_
...
namespace Eval::NNUE {
    ...
    class Trainer<Layers::SemiAffineTransform<PreviousLayer, InputStride, OutputDimensions>> {
    private:
        // Type of layer to learn
        using LayerType = Layers::SemiAffineTransform<PreviousLayer, InputStride, OutputDimensions>;
                ...
                for (IndexType i = 0; i < kOutputDimensions; ++i) {
                    double sum = 0.0;
                      for (IndexType j = 0; j < kInputStride; ++j) {
                          ...
                          weights_[kInputStride * i + j] = weight;
                          sum += weight;
                      }
        ...
        const LearnFloatType* propagate(ThreadPool& thread_pool, const std::vector<Example>& batch) {
            ...
            for (IndexType block = 0; block < kNumBlocks; ++block)
            {
                cblas_sgemm(
                    ...
                    kOutputStride, batch_size_, kInputStride,
                    1.0,
                    weights_ + kInputStride * kOutputStride * block, kInputStride,
                    batch_input_ + kInputStride * block, kInputDimensions,
                    1.0,
                    &output_[block * kOutputStride], kOutputDimensions
                );
            }
#else
        ...
        void backpropagate(ThreadPool& thread_pool,
            ...
            for (IndexType block = 0; block < kNumBlocks; ++block)
            {
                cblas_sgemm(
                    ...
                    kInputStride, batch_size_, kOutputStride,
                    1.0,
                    weights_ + kInputStride * kOutputStride * block, kInputStride,
                    gradients + kOutputStride * block, kOutputDimensions,
                    0.0,
                    &gradients_[kInputStride * block], kInputDimensions
                );
            }
            ...
            for (IndexType block = 0; block < kNumBlocks; ++block)
            {
                cblas_sgemm(
                    ...
                    kOutputStride, kInputStride, batch_size_,
                    1.0,
                    gradients + kOutputStride * block, kOutputDimensions,
                    batch_input_ + kInputStride * block, kInputDimensions,
                    momentum_,
                    weights_diff_ + kInputStride * kOutputStride * block, kInputStride
                );
            }
            ...
            for (IndexType i = 0; i < kOutputDimensions * kInputStride; ++i) {
                const double d = local_learning_rate * weights_diff_[i];
                weights_[i] -= d;
                abs_weights_diff_sum_ += std::abs(d);
            }
            num_weights_diffs_ += kOutputDimensions * kInputStride;
    ...
    private:
        ...
        void quantize_parameters() {
            ...
            for (IndexType i = 0; i < kOutputDimensions; ++i) {
                const auto offset = kInputStride * i;
                const auto padded_offset = LayerType::kInputStride * i;
                for (IndexType j = 0; j < kInputStride; ++j) {
        ...
        static constexpr IndexType kInputStride = LayerType::kInputStride;

        static constexpr IndexType kNumBlocks = LayerType::kNumBlocks;

        static constexpr IndexType kOutputStride = LayerType::kOutputStride;
        ...
        alignas(kCacheLineSize) LearnFloatType weights_[kOutputDimensions * kInputStride];

        // Buffer used for updating parameters
        alignas(kCacheLineSize) LearnFloatType biases_diff_[kOutputDimensions];
        alignas(kCacheLineSize) LearnFloatType weights_diff_[kOutputDimensions * kInputStride];
