...
namespace Eval::NNUE::Features {
    ...
    inline IndexType HalfKPE9E_KingRing<AssociatedKing>::make_index(
        Color perspective,
        Square s,
        Piece pc,
        Square ksq,
    ...
    template <Side AssociatedKing>
    void HalfKPE9E_KingRing<AssociatedKing>::append_active_indices(
        const Position& pos,
        Color perspective,
        IndexList* active) {

        Square actual_ksq = pos.square<KING>(
        ...
        Bitboard king_ring = PseudoAttacks[KING][actual_ksq];
        Bitboard* mobility = pos.state()->mobility;
        Bitboard* mobility2 = pos.state()->mobility2;
        Bitboard bb =
            ((mobility[WHITE] | mobility[BLACK]) & king_ring)
            | pos.pieces();
        while (bb) {
            Square s = pop_lsb(&bb);
    ...
    template <Side AssociatedKing>
    void HalfKPE9E_KingRing<AssociatedKing>::append_changed_indices(
        const Position& pos,
        Color perspective,
        IndexList* removed,
        IndexList* added) {
        ...
        Bitboard prev_mobility_any = (prev_mobility[WHITE] | prev_mobility[BLACK]) & king_ring;
        Bitboard curr_mobility_any = (curr_mobility[WHITE] | curr_mobility[BLACK]) & king_ring;
    ...
    template class HalfKPE9E_KingRing<Side::kFriend>;
    template class HalfKPE9E_KingRing<Side::kEnemy>;
