...
namespace Eval::NNUE::Features {
    ...
    inline IndexType HalfQA<AssociatedKing>::make_index(
        Color perspective,
        Square s,
        Piece pc,
        Square ksq) {
    ...
    template <Side AssociatedKing>
    void HalfQA<AssociatedKing>::append_active_indices(
        const Position& pos,
        Color perspective,
        IndexList* active) {
        ...
        if (pos.count<QUEEN>(c) == 0)
            return;

        Square ksq = orient(
            perspective,
            pos.squares<QUEEN>(c)[0]);
    ...
    template <Side AssociatedKing>
    void HalfQA<AssociatedKing>::append_changed_indices(
        const Position& pos,
        Color perspective,
        IndexList* removed,
        IndexList* added) {
    ...
    template class HalfQA<Side::kFriend>;
    template class HalfQA<Side::kEnemy>;
