...
namespace Stockfish::Eval::NNUE::Features {
    ...
    inline IndexType HalfKA_E3<AssociatedKing>::make_index(
        Color perspective,
        Square s,
        Piece pc,
        int bucket,
        ...
        mobility_index = std::clamp(mobility_index, 0, 2);

        return IndexType(orient(perspective, s) + kpp_board_index[perspective][pc] + PS_END2 * bucket)
            + (static_cast<IndexType>(SQUARE_NB) * static_cast<IndexType>(PS_END2) / 2 * static_cast<IndexType>(mobility_index));
    }

    template <Side AssociatedKing>
    inline std::pair<IndexType, IndexType> HalfKA_E3<AssociatedKing>::make_index_2(
        Color perspective,
        Square s,
        Piece pc,
        int bucket,
        ...
        int prev_mobility_index = 1;
        int curr_mobility_index = 1;
        ...
        prev_mobility_index = std::clamp(prev_mobility_index, 0, 2);
        curr_mobility_index = std::clamp(curr_mobility_index, 0, 2);

        const IndexType index_base = IndexType(orient(us, s) + kpp_board_index[us][pc] + PS_END2 * bucket);
        constexpr IndexType mobility_index_mul = static_cast<IndexType>(SQUARE_NB) * static_cast<IndexType>(PS_END2) / 2;
    ...
    template <Side AssociatedKing>
    void HalfKA_E3<AssociatedKing>::AppendActiveIndices(
        const Position& pos,
        Color perspective,
        IndexList* active) {
        ...
        while (bb) {
            Square s = pop_lsb(&bb);
            active->push_back(make_index(perspective, s, pos.piece_on(s), bucket, curr_mobility));
        }
    }

    // Get a list of indices for recently changed features
    template <Side AssociatedKing>
    void HalfKA_E3<AssociatedKing>::AppendChangedIndices(
        const Position& pos,
        Color perspective,
        IndexList* removed,
        IndexList* added) {
        ...
        for (int i = 0; i < dp.dirty_num; ++i) {
            ...
            {
                updated |= dp.from[i];
                removed->push_back(make_index(perspective, dp.from[i], pc, bucket, prev_mobility));
            }

            if (dp.to[i] != SQ_NONE)
            {
                updated |= dp.to[i];
                added->push_back(make_index(perspective, dp.to[i], pc, bucket, curr_mobility));
            }
        }
        ...
        while (affected_pieces)
        {
            ...
            auto [r, a] = make_index_2(perspective, s, pc, bucket, prev_mobility, curr_mobility);
            if (r != a)
            {
                removed->push_back(r);
                added->push_back(a);
    ...
    template class HalfKA_E3<Side::kFriend>;
