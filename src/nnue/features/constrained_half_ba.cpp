...
namespace Eval::NNUE::Features {
    ...
    inline IndexType ConstrainedHalfBA<AssociatedKing>::make_index(
        Color perspective,
        Square s,
        Piece pc,
        Square ksq) {
    ...
    template <Side AssociatedKing>
    void ConstrainedHalfBA<AssociatedKing>::append_active_indices(
        const Position& pos,
        Color perspective,
        IndexList* active) {
        ...
        auto queens = pos.pieces(c, BISHOP);
        ...
        for (int j = 0; j < 2 && queens; ++j)
        {
            ...
            Bitboard influence = PseudoAttacks[BISHOP][ksq];

            // We exclude our king as this feature should be covered by halfka
            Bitboard bb = pos.pieces() & ~pos.pieces(KING) & ~pos.pieces(QUEEN) & influence;
    ...
    template <Side AssociatedKing>
    void ConstrainedHalfBA<AssociatedKing>::append_changed_indices(
        const Position& pos,
        Color perspective,
        IndexList* removed,
        IndexList* added) {
        ...
        for (int j = 0; j < 2 && queens; ++j)
        {
            ...
            for (int i = 0; i < dp.dirty_num; ++i) {
                ...
                if (type_of(pc) == KING || type_of(pc) == QUEEN)
                    continue;
    ...
    template class ConstrainedHalfBA<Side::kFriend>;
    template class ConstrainedHalfBA<Side::kEnemy>;
