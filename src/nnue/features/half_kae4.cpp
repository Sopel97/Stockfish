...
namespace Eval::NNUE::Features {
    ...
    inline IndexType HalfKAE4<AssociatedKing>::make_index(
        Color perspective,
        Square s,
        Piece pc,
        Square ksq,
        ...
        return IndexType(orient(perspective, s) + kpp_board_index[pc][perspective] + PS_END2 * ksq)
            + (static_cast<IndexType>(SQUARE_NB) * static_cast<IndexType>(PS_END2)) * mobility_index;
    }

    // Get a list of indices for active features
    template <Side AssociatedKing>
    void HalfKAE4<AssociatedKing>::append_active_indices(
        const Position& pos,
        Color perspective,
        IndexList* active) {
    ...
    template <Side AssociatedKing>
    void HalfKAE4<AssociatedKing>::append_changed_indices(
        const Position& pos,
        Color perspective,
        IndexList* removed,
        IndexList* added) {
    ...
    template class HalfKAE4<Side::kFriend>;
    template class HalfKAE4<Side::kEnemy>;
